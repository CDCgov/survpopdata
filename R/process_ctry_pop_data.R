# Private functions ----
#' Load Pakistan country population patch
#'
...
load_pakistan_patch <- function(
  ...
  sirfunctions::sirfunctions_io("read", NULL, pakistan_file_path, edav = edav) |>
    ...
    dplyr::select(Country, `2022`, `2023`) |>
...
#' Load Somalia country population patch
#'
...
#' Load Kenya country population patch
#'
...
#' Load Jamal country under-15 population
#'
...
load_jamal_pop <- function(
  ...
  jamal_pop_formatted <- jamal_pop |>
    dplyr::mutate(
      ...
      adm0guid = paste0("{", toupper(PlaceId), "}"),
      Admin0Name = ifelse(stringr::str_detect(Admin0Name, "IVOIRE"), "COTE D IVOIRE", Admin0Name)
    ) |>
    dplyr::select(ends_with("Name"), starts_with("0"), adm1guid) |>
    tidyr::pivot_longer(
      cols = starts_with("0"),
      names_to = "year",
...
#' Combine all country patch sources
#'
...
#' Join named population rows to country-year shapes
#'
#' @param pop_named Tibble with Admin names, year, age columns, datasource.
#' @param country_long Country-year long shapes table.
#'
#' @return Tibble joined to year-valid country shapes.
#'
#' @export
join_pop_to_country_year_shapes <- function(pop_named, country_long) {
  pop_named |>
    dplyr::left_join(
      country_long,
      by = c(
...
#'
#' @param non_polis_pop Tibble with ADM0_GUID, year, age columns, datasource.
#'
...
remove_forward_fill_non_polis <- function(non_polis_pop) {
  non_polis_pop |>
    ...
    dplyr::arrange(ADM0_GUID, datasource, year) |>
    dplyr::group_by(ADM0_GUID, datasource) |>
    dplyr::mutate(
      Total = dplyr::if_else(
...
#'
#' @param pop_with_guid Tibble with ADM0_GUID, year, age columns, datasource.
#'
#' @return Deduplicated tibble (one row per ADM0_GUID-year).
#'
#' @export
...
#' Aggregate province population to country for missing values
#'
#' Fills remaining country-level NAs by summing all provinces in that country-year.
#' If any province is still NA, triggers aggregate_districts_to_province() first.
#'
#' @param base_data Tibble of country-level population data with remaining NAs post growth-rate fill.
#'
#' @return Tibble with remaining NAs filled and aggregation flags.
#' @keywords internal
#'
aggregate_provinces_to_country <- function(base_data) {

  # Retrieve province-level population
  province_data <- process_prov_pop_data()

  # If any province still has NAs, fill via district aggregation first
  provinces_with_nas <- province_data |>
    dplyr::filter(is.na(Total) | is.na(Under5Pop) | is.na(Under15Pop))

  if (nrow(provinces_with_nas) > 0) {
    province_data <- province_data |>
      dplyr::rows_update(
        aggregate_districts_to_province(provinces_with_nas),
        by = c("ADM0_NAME", "ADM1_NAME", "year")
      )
  }

  # Aggregate (sum) all provinces to country-year level
  country_aggregates <- province_data |>
    dplyr::group_by(ADM0_NAME, year) |>
    dplyr::summarise(
      Total_from_provinces      = sum(Total, na.rm = TRUE),
      Under5Pop_from_provinces  = sum(Under5Pop, na.rm = TRUE),
      Under15Pop_from_provinces = sum(Under15Pop, na.rm = TRUE),
      .groups = "drop"
    )

  base_data |>
    dplyr::left_join(country_aggregates, by = c("ADM0_NAME", "year")) |>
    dplyr::mutate(
      used_aggregate_Total      = is.na(Total)      & !is.na(Total_from_provinces),
      used_aggregate_Under5Pop  = is.na(Under5Pop)  & !is.na(Under5Pop_from_provinces),
      used_aggregate_Under15Pop = is.na(Under15Pop) & !is.na(Under15Pop_from_provinces),
      Total      = dplyr::if_else(used_aggregate_Total,      Total_from_provinces,      Total),
      Under5Pop  = dplyr::if_else(used_aggregate_Under5Pop,  Under5Pop_from_provinces,  Under5Pop),
      Under15Pop = dplyr::if_else(used_aggregate_Under15Pop, Under15Pop_from_provinces, Under15Pop)
    ) |>
    dplyr::select(-Total_from_provinces, -Under5Pop_from_provinces, -Under15Pop_from_provinces)
}
